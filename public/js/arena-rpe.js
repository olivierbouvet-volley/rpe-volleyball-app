...
function renderRpe() {
  ...
  container.innerHTML = `
    ${arenaHeader({
      ...
      title: 'Log RPE',
      right: `<span style="font-family:var(--arena-mono);font-size:10px;color:var(--arena-text-muted);padding:4px 8px;border:1px solid var(--arena-border);border-radius:4px;">STEP ${s.step}/4</span>`,
    })}
    ...
    <div style="padding: 14px 16px 110px; overflow-y: auto; height: calc(100vh - 200px);">
      ...
      ${renderDurationCard()}
      ${s.step >= 3 && s.rpe > 0 ? renderChargeBlock(charge) : ''}
      ${s.step >= 4 ? renderRpeComment() : ''}
      ${s.step >= 3 && s.rpe > 0 ? renderRpeSubmit() : ''}
    </div>

    ${arenaTabBar({ activeTab: 'rpe' })}
  `;
